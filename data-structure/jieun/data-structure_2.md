...
- 레코드 색인 키가 하나인 경우 -> 일차원 검색 트리
- 하나의 키가 여러 개의 필드를 사용하는 경우 -> 다차원 검색 트리

## 5.1. KD-Tree 란?

<img width="810" height="394" alt="Image" src="https://github.com/user-attachments/assets/79e0f658-4e2c-4b76-98c2-8039ffc145ef" />

- 이진 검색 트리를 확장한 것으로, `k(k≥2)`개의 필드로 이루어진 키를 사용한다.
- 각 레벨(Level)에서 필드를 번갈아가며 검색에 사용한다.
    - 한 레벨에서는 하나의 필드만 사용한다.
    - 총 k개의 필드를 사용하는 검색이라면, k개의 레벨을 내려가면 검색에 사용하는 필드가 일치한다.

## 5.2. KD-Tree의 특징
- Root (Level 0): 첫 번째 필드(예: x축)만 사용하여 분기한다.
- Level 1: 두 번째 필드(예: y축)만 사용하여 분기한다.
- Level i: i (mod k) 번째 필드를 사용하여 분기한다.

## 5.3. KD-Tree의 연산
1. KD-Tree의 **검색**
- 임의의 키가 입력되면, 각 필드를 차례대로 사용해서 트리를 검색한다.

2. KD-Tree의 **삽입**
- 삽입은 리프 노드에서 이루어진다.
- 트리를 따라 내려가다 리프 노드를 만나면 왼쪽 또는 오른쪽에 삽입한다.

3. KD-Tree의 **삭제** (노드 r을 삭제한다는 가정)
- 자식이 없는 경우
    - BST와 마찬가지로 노드 r만 제거한다.
- 자식이 하나뿐인 경우 
    - 자식 노드를 루트로 하는 서브 트리가 한 레벨 위로 이동하므로 분기에 사용되는 필드가 달라져 트리의 성질이 깨지게 된다.
    - 자식의 둘인 경우와 동일한 방법으로 대체 노드를 찾아 처리한다.
- 자식이 둘인 경우
    - 오른쪽 서브 트리가 있는 경우: 삭제하려는 노드 r의 현재 레벨 분기 기준 값이 가장 작은 노드를 오른쪽 서브트리에서 찾아 대체한다.
    - 왼쪽 서브 트리만 있는 경우: 분기 기준 값이 가장 큰 노드를 왼쪽 서브트리에서 찾아 대체한다.
